 ...
 * */
 class TransactionsPage {
   ...
   * */
  registerEvents() {
    const deleter = document.querySelector('.content-wrapper');
  ...
  removeAccount() {
    if (this.lastOptions) {
      if (window.confirm('Удалить счёт и все транзакции?')) {
        this.clear();
   ...
   * */
  renderTitle(name){
    const userName = document.querySelector('.content-title');
    userName.textContent = '';
    userName.insertAdjacentText('afterbegin', name);
  }
  ...
  renderTransactions(dataList){
    ...
    if (dataList) {
      dataList.forEach(key => transactionsList.insertAdjacentHTML('beforeend', this.getTransactionHTML(key)));   
    }
    else {
